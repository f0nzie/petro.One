...
In this example we are requesting only the `conference-paper` type. Then, we add the parameter `dc_type` to `make_search_url`.

Note also that we are adding another parameter `rows` to get 1000 rows instead of 10, 50 or 100 as the browser only allows.

> Note. the maximum number of rows that can be retrieved at a time from OnePetro is `1000`.
...
#### Word cloud for conference papers
We plot the dataframe that was obtained above.
...
get_papers_count(my_url)
df <- read_multidoc(my_url, verbose = TRUE)
...
## Search for **any** word
In this other example, we want papers that contain the word "well" or "test".
